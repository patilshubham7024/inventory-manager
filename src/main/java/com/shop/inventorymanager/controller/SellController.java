...
public class SellController {
    ...
    @GetMapping
    public List<SellOrderDTO> get(@RequestParam(required = false) Optional<LocalDateTime> from,
    ...
    @PostMapping
    public SellAddResponse add(@RequestBody @Valid SellAddRequest sellAdd) {
        log.info("SellController -> add");
        SellOrder added = sellService.add(sellAdd);
        if(added != null) {
        return SellAddResponse.builder().message(SELL_ADD_SUCCESS).build();
        }
